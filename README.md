...
> **前置条件**：手势识别依赖 MediaPipe，模型文件通过 CDN 动态加载（约 10MB+），请确保**网络畅通**，离线环境下手势功能将不可用，但照片上传等基本功能仍可正常使用。
...
注：持续五指并拢使爱心缩小至 **50% 以下**（`targetScale < 0.5`）时，触发粒子爆炸效果；若此时已上传照片，照片将以 **2× 大小**弹出展示。握拳可取消当前选中的照片以便选择下一张。
